...
class sql_data():
    ...
    def _query_for_all_data(self, table:str) -> Optional[str]:
        ...
        if table=='api_oldcustomer':
            ...
            f'api_purpose.id AS purpose_id, api_state.id AS addr_state_id '\
            f'FROM {table} LEFT JOIN api_homeownership '\
            f'ON {table}.home_ownership{org_suff}=api_homeownership.name LEFT JOIN '\
            f'api_purpose ON {table}.purpose{org_suff}=api_purpose.name LEFT JOIN '\
            f'api_state ON {table}.addr_state{org_suff}=api_state.name LEFT JOIN '\
    ...
    def import_all_data(self, table:str) -> pd.DataFrame:
        ...
        self._query_additional_data(table)
...
# TO-DO: api_state values start with USA, whereas
...
for table in desired_tables:
    ...
    # Fix wrong quantities and assing proper format to columns
...
# Perform NaN treatment
cols_with_nans = [col for col in df.columns if df[col].isnull().any()]
print(f'The following columns have NaNs, {",".join(cols_with_nans)}')
